...
c               derivatives with respect to the radial coordinate
c               x for a range of values of the order m and lnum
c               values for the degree l = m, m+1,...,m+lnum-1 for
c               a specified complex value of the size parameter c.
c               To calculate the first kind oblate angular
c               functions s1 and their first derivatives with
c               respect to the angle coordinate eta for a range
c               of values of m, l, and eta for a specified complex
c               c.
c
c  This program for complex c is a major advance in capability
c  from coblfcn version 1.01 and an improvement over version 1.04.
...
c  and d is the interfocal distance of the elliptical cross-section
...
c  of arithmetic is set in the module param located above right before
c  coblfcn. For some compilers, the module param must be located before
c  coblfcn. For others it can be located right after coblfcn. Here, the
c  kind parameters knd and knd1 are set by the statements:
...
c
c  An alternative to using module param is to create three versions of
c  coblfcn, one for double precision, one for quadruple precision and
c  one for the hybrid choice. To do this, replace knd and knd1
c  throughout coblfcn with the appropriate values of kind. Also remove
c  all occurrences of the statement 'use param'.
c
...
c  They are presently set to 8 and 16, respectively. Increasing the
c  values of kindd and kindq will improve the accuracy and can
c  significantly extend the paramteter ranges over which coblfcn
c  provides useful results.
c
...
c  and quadruple precision arithmetic, respectively.c  
c
c  A description of the ranges of parameters for which coblfcn provides
c  useful results is given following the discussion of input and output
c  files. Following that is a discussion of how to obtain the expansion
c  d coefficients and the various angular function normalizations.
c
c          Input data
c
c  Input parameters are read from unit 1 in the file coblfcn.dat
c  assumed to be located in the directory of the fortran file.
c  coblfcn.dat contains the following lines of data. (Note that a
c  subroutine version of coblfcn is available where the input parameters
c  and output function values are passed through the call statement).
c
c       line 1:
...
c                   should be an even integer; if lnum is odd coblfcn
c                   will add one to lnum to make it even
c
c       line 2:
c          ioprad : (integer)
...
c
c       line 3:
c          c      : complex value of the size parameter (= kd/2,
c                   where k = the complex wavenumber and d =
c                   interfocal length) (either complex*16 or complex*32).
c                   Note that later in the subroutines cc is used to
c                   denote the complex value for c, and c is used to
c                   denote its amplitude (magnitude).
c          x      : value of the radial coordinate x (real*8 or
c                   real*16 format). (any value can be entered if
c                   ioprad = 0)
c
c       line 4:
c          ioparg : (integer)
...
c                   functions are to be computed. (real*8 or real*16
c                   format)
c
c          darg   : increment used to calculate additional desired
c                   arguments for angular functions. (real*8 or real*16
c                   format)
c
...
c          Output Data
c
c  Coblfcn is designed around the number of decimal digits ndec and
c  the maximum exponent nex available in the desired arithmetic on
c  the user's computer. It is written to avoid overflow and underflow
c  and can provide radial function values far outside the range
c  10**(-nex) and 10**(nex). It does this by expressing each radial
c  function in scientific notation and giving it in two parts. The
c  first part, called the characteristic, with both a real and an
c  imaginary component, has a magnitude ranging from unity to just
c  less than 10.0. The characteristic contains the ndec decimal digits
c  in both components of the function value. The second part is the
c  integer exponent (power 10). For example, the radial function of the
c  first kind is given by the complex characteristic r1c and the
c  integer ir1e. The corresponding radial function value is then equal
c  to r1c*(10.0e0_knd**ir1e). The radial function values can fall
c  outside the dynamic range of floating point numbers at relatively
c  large orders where the functions of the first kind would underflow
c  and the corresponding functions of the second kind would overflow.
c
c  The angular functions are also given as a characteristic and an
c  integer exponent. If the angular functions are normalized according
c  to Meixner-Schafke to have the same norm as the corresponding
c  associated Legendre functions, their exponent can be very large
c  for high values of the order m. In this case the use of a
c  characteristic and exponent can prevent overflow. Coblfcn also
c  provides the option for the angular functions to have unity norm,
c  a convenient choice that prevents overflow if the exponents and
c  characheristics are combined to form angular function values. This
c  choice also eliminates the need to otherwise compute the norm that
c  appears in solutions of problems involving oblate functions.
c
...
c  fort.50, can be suppressed by searching coblfcn for write(50 and
...
c  blocksize or perhaps change the status to append for one or more of
c  the files.
c
c   fort.20
c
...
c     in the radial functions as measured using the Wronskian or
c     estimated using subtraction errors in the calculations. [see
c     comments below regarding naccr]
c
...
c                        larger than those allowed in the arithmetic
c                        used for the calculations since the
c                        floating point function values provided
c                        are given as a characteristic and an
c                        integer exponent. Use of ratios in
...
c                        integer naccr. r1 and r1d are usually very
c                        accurate so that naccr relects the accuracy
c                        of r2 and r2d.
c
...
c                        the estimated accuracy of r1 and r1d.
c
...
c                        less accurate the larger the value of cr, the
...
c     They are annotated and should be self explanatory. If desired,
c     these files can be suppressed by searching for write(40 and
c     write(50 and commenting out the lines the lines of code found.
c
c   fort.60
c
c     This file should be of interest to the user, especially when using
c     coblfcn outside the ranges where useful results are expected or
c     when using a compiler that provides fewer decimal digits than 15
c     in double precision or 33 in quadruple precision. See the
c     discussion of these ranges below. Whenever the estimated accuracy
c     of the radial functions falls below a designated integer value
c     during the running of coblfcn, the associated values of x, c, m,
c     and l are written to fort.60. This integer is currently set equal
c     to 6 in write statements found after the line numbered 1405 below
c     in subroutine main. I expect that accuracies as low as 5 or 6
c     igits and possibly even as low as 4 digits may be sufficient in
c     many cases, especially when they occur very rarely. The integer
c     can be set higher if desired.
c
...
c     function accuracy. It provides the values of m, l, and c where
c     this occurs together with the value ndec - jsubms -1, where jsubms
c     is the subtraction error in calculating the normalization.
c
c     Fort.60 also alerts the user to the unlikely situation where the
c     eigenvalue procedure has failed for one or more values of l and
c     the same eigenvalue is obtained for two different values of l
c     (of the same parity). In this case one or more eigenvalues are
c     missing and the results are not usable. I do not expect this to
c     be a possibility unless coblfcn is used with ci and cr beyond the
c     ranges stated above for which useful results are expected.
c
c
...
c  precision if it is available. Note that this might increase the run
c  time by a factor of 50 or more.
c
...
c  precision arithmetic. More recently, coblfcn was also tested using a
c  compiler from Absoft that also provided 15 decimal digits for double
c  precision but provided 31 digits of precision for quadruple
c  precision. The discussion below will include both the 33 digit and
c  31 digit results for quadruple precision. If the user's computer
c  provides a different number of digits for either double or quadruple
c  precision, the following estimates might need to be adjusted up or
c  down depending on whether more or fewer digits are provided.
c
c  The estimated accuracy of the resulting function values is given
c  below in terms of decimal digits. It is often obtained using the
c  comparison of the theoretical value for the Wronskian and its value
c  obtained using the computed radial function values. But it can be an
c  estimate based on subtraction errors in the calculation. See the
c  discussion for the accuracy integer naccr in the section for fort.20.
c
c  Testing included values of x ranging from 0.000001 to 10 as well as
c  the special case of x = 0, values for cr up to 5000, and values of ci
c  up to 200. Testing for both the double precision and hybrid versions
c  included all values of the order m from 0 to 200 and from 210 to 1000
c  in steps of 10. Testing for the quadruple precision version was the
c  same for cr less than about 1000. Testing for larger cr included
c  values of m from 0 to 200 in steps of 10 and from 250 to 1000 in
c  steps of 50. For all versions the values of the degree l ranged from
c  m to m + lnum -1, where lnum was chosen sufficiently large that the
c  magnitudes of r1 and r1d were less than 10**(-300).
c
...
c  results for small x. At least 5 digits were obtained for cr up to
c  5000 for all of the values of x down to 0.000001.
c
...
c
c  When ci = 20, there were 5 or more digits of accuracy for all tested
c  values of x when cr <= 95. There were 5 or more digits of accuracy at
c  x >= 0.1 for cr = 150 and at x >= 0.2 for cr up to 2000. Testing
...
c  than it does for 128 bit arithmetic.
c
c  When there were 33 digits of precision, testing for values of ci up
c  to 40 showed that coblfcn provides useful results for cr up to at
c  least 5000 and for m up to 1000 and x down to 0.000001. Estimated
c  accuracies for the radial functions were 8 or more digits except for
c  a possible rare 6 or 7 digit result. When there were 31 digits of
c  precision, useful results were obtained over the same ranges of m
c  and x and for cr up to about 4000. There were some scattered 5 digit
c  results. Results for cr = 5000 showed a few isolated 4 digit results
c  occurring primarily for x from about 0.009 to 0.025. See the
c  discussions of estimated accuracies given below for the special case
c  of x = 0 and for the angular functions.
c
c  Testing for ci = 50 with 33 digits of precision showed useful results
c  for x down to 0.000001 and cr up to at least 2000. Accuracies for the
c  radial functions were almost always at least 8 or more digits but
c  occasional accuracies as low as 5 digits were seen near the
c  breakpoint, primarily for cr >= 1000 and m > 200. When cr = 2000,
c  there were even a few 4 digit results for m >= 700 and a few 3 digit
c  results for m >= 800. It is unlikely that values of m this large will
c  be required by the user. When there were 31 digits of precision,
c  testing showed useful results for x down to 0.000001 and cr up to at
c  least 600. Accuracies were usually 6 or more digits with a rare 5
c  digit result for m less than 200. Above m = 200, there were a number
c  of 5 digit results and even an occasional 4 digit result for m
c  between 200 and 350. There were also useful results for cr up to 2000
c  when x was no larger than about 0.01 and m was no larger than about
c  600.  
c
c  Testing for ci = 60 with 33 digits of precision showed useful results
c  for all m and x>= 0.000001 with cr up to 100 although a few 4 digit
c  results occurred at x = 0.00001 and 0.000001. Results for cr = 200
c  were similar except that there were a few more 4 digit results at x
c  = 0.000001 and 0.00001 and some 4 digit results now at x = 0.0001 and
c  x = 0.001. Testing for cr = 500 showed that useful results are only
c  obtained for x >= 0.05 for all m and for x < 0.05 for m up to about
c  160. Testing for cr = 1000 and 2000 showed useful results for x >=
c  0.2 for all m with a possible 4 digit result or so. Testing with 31
c  digits of precision showed useful results for all m for cr up to 150
c  when x >= 0.01, for cr up to 300 when x >= 0.1 and for cr up to 1500
c  when x >= 0.5.
c
c  Testing for ci = 70 with 33 digits of precision showed useful results
c  for all m for cr up to 100 when x >= 0.01, for cr up to 150 when x >=
c  0.05, for cr up to 200 when x >= 0.1, for cr up to 500 when x >= 0.3
c  and for cr up to 1000 when x >= 0.5. Testing with 31 digits of
c  precision showed useful results for all m for cr up to 75 when x >=
c  0.1, for cr up to 200 when x >= 0.2 and for cr up to 500 when x >=
c  0.5, although a rare 4 digit result occurred sometimes.
c
c  Testing for ci = 80 with 33 digits of precision showed useful results
c  for cr up to 20 when x >= 0.01, for cr up to 50 when x >= 0.02, for
c  cr up to 100 when x >= 0.2, for cr up to 200 when x >= 0.3 and for ci
c  up to 300 when x >= 0.5. Testing with 31 digits of precision showed
c  useful results for all m for cr up to 30 when x >= 0.2, for cr up to
c  180 when x >= 0.3 and for cr up to 250 when x >= 0.5, although a rare
c  4 digit result occurred sometimes. 
c
c  Testing for yet higher values of ci showed a continued increase in
...
c
        use param
c
        real(knd) ca,arg1,darg,step1,step2,step3,x,xneu
c
        complex(knd) c
c
...
c  These can be changed if desired. See comments above about changing
c  minacc
c
...
c  set array dimensions
        ...
        if(lnum.lt.nbp.and.(2*(lnum/2).ne.lnum)) lnum=lnum+1
...
c
     ...
     1             x,ngau,step1,nstep1,step2,nstep2,step3,nstep3,
     2             ioparg,arg1,darg,narg,maxd,maxdr,maxe,maxe2,maxint,
     3             maxj,maxlp,maxm,maxmp,maxn,maxp,maxpdr,maxq,maxt,
     4             neta,jnenmax,jnebmax,xneu,ndec,nex,kindd,kindq)
c
        end
c
c
...
c                        functions
c               x      : radial coordinate (shape parameter)
...
c  real(knd) and complex(knd) scalars
        real(knd) aj1,aj2,apcoef,apcoefn,api,arg1,c,coefn,coefme,
     1            coefmo,darg,dec,em,etaval,factor,factor1,pcoefn,pi,
     2            qdm0,qdm1,qm0,qm1,rm,rm2,step1,step2,step3,ten,termpq,
     4            teste,testeo,t1,t2,t3,t4,t5,t6,t7,wm,x,xb,xbninp,xl,
     5            xhigh,xlow,xneu
...
c
     ...
     3               r1dcm,r1d1c,r1ec,r1dec,r2ic,r2dic,r2lc,r2dlc,
     4               r2l1c,r2dl1c,r2nc,r2dnc,r2ec,r2dec,wronc,wronca,
     5               wroncb,wront
...
c  integer and real(knd) arrays with dimension maxlp
     ...
     1          ibesesv(jnebmax,maxlp)
        real(knd) pnormint(maxlp)
c
c  complex(knd) arrays with dimension maxlp
        complex(knd) sbesdr(maxlp),sbesn(maxlp),sneudr2(maxlp),
     1               sneudre(maxlp),sneudrsv(jnenmax,maxlp),
     2               sbesne(maxlp),sbesdre(maxlp),
     3               sbesnsv(jnebmax,maxlp),sbesdrsv(jnebmax,maxlp)
c
...
c  complex(knd) arrays with dimension maxn
     ...
     1               sneudfsv(jnenmax,maxn),sneuf2(maxn),sneudf2(maxn),
...
350           continue
     ...
     1                                 bliste1,gliste1,ndec,ndec1,maxd,
     2                                 ioprad,minacc,eigest,eigmat,lipl,
     3                                 lips,liplp,mat,eigp,eign,kindd,
     4                                 kindq,eigval,enr,ienre,itestme,
     5                                 naccre,ieigt,iopeige,iflage,
     6                                 kflag)
              if(ix.eq.1) call conver (l,m,lnum,cc,limd,blisto,glisto,
     1                                 blisto1,glisto1,ndec,ndec1,maxd,
     2                                 ioprad,minacc,eigest,eigmat,lipl,
     3                                 lips,liplp,mat,eigp,eign,kindd,
     4                                 kindq,eigval,enr,ienro,itestmo,
...
c  determine oblate radial functions of both kinds when x = 0
     ...
     2                 dneg,idneg,nsdneg,nsdrhor1, ndshro)
...
530           format(12x,'r2 is accurate to ',I2,' decimal digits; r1,'
     1               ' r1d, and r2d are accurate to ',i2,' decimal'
     2               '  digits.')
              if(ix.eq.1) write(40,540) naccr,naccr1
540           format(12x,'r2d is accurate to ',I2,' decimal digits. r1,'
     1               ' r1d, and r2 are accurate to ',i2,' decimal'
     2               '  digits.')
              go to 1330
550           continue
c
...
790           if(iopint.eq.1.and.ipint.eq.1) iopint=2
     ...
     4                  coefme,coefmo)
              iopint=2
800           continue
              if(iopint.eq.2) limint=maxint-6
...
900           continue
     ...
     3                   nsdneg,dfnorm,idfe,dmfnorm,idmfe,prx,pdrx,
     4                   qdrat,qdqr,qdm1,iqdm1,qdl,iqdl,qrat,qm1,iqm1,
     5                   ql,iql,fajo,ifajo,ifsub,jsub,termpq,itermpq,
     6                   ioppsum,iopqnsum,r1cin,ir1ein,r1dcin,ir1dein,
     7                   naccr1,naccrpl,itestm,r2lc,ir2le,r2dlc,ir2dle,
     8                   jleg,jlegp,jflagl,naccleg,kflagl,isub,isubd,
     9                   nacccor)
              iopleg=2
...
c     equal to 0
     ...
     1                    sneuf2,sneun2,ineue2,sneudf2,sneudr2)
              ibflag2=0
1040          continue
              iopneu0=iopneu0+1
1050          continue
...
1160          continue
     ...
     1                    sneufe,sneune,ineuee,sneudfe,sneudre)
                do je=1,limn
...
1340            format(1x,i5,2x,2(f17.14,1x,f17.14,i6,2x),i2)
                go to 1400
                end if
...
1350            format(1x,i5,2x,2(f17.14,1x,f17.14,i6,2x),/,8x,
     1                   2(f17.14,1x,f17.14,i6,2x),i2,'e')
                go to 1400
                end if
...
1380          format(1x,i5,2x,2(f17.14,1x,f17.14,i6,2x),/,8x,
     1                   2(f17.14,1x,f17.14,i6,2x),i2,'w')
1390          continue
                if(ix.eq.1) then
...
c  determine first kind oblate angular functions
     ...
     1                   maxt,maxd,maxp,enr,dmlms,idmlmse,jsubms,pr,pdr,
     2                   pdnorm,ipdnorm,pnorm,ipnorm,pdtempe,ipdtempe,
     3                   pdtempo,ipdtempo,ptempe,iptempe,ptempo,iptempo,
     4                   itestm,naccre,kindd,kindq,s1c,is1e,s1dc,is1de,
     5                   naccs,naccds,jang,dmlms1,idmlms1e)
                do 1500 jarg=1,narg
...
1435            format(1x,'eta = ',e24.15,'   s1 and s1d accuracy = ',
     ...
     1                s1c(jarg),is1e(jarg),s1dc(jarg),is1de(jarg)
1440            format(1x,f17.14,2x,f17.14,1x,f17.14,2x,i5,2x,' ,',
     1                 i2)
1450            format(1x,f17.14,2x,f17.14,1x,f17.14,2x,i5,/,20x,f17.14,
...
c  complex(knd) scalars and arrays
     ...
     1               r1dc,r1temp,r1tempa,r1dtemp,r1dtempa,r1top,r1topa,
     2               r1dtop,term
...
c  real(knd) scalars and arrays
     ...
     1             r2dposr,r2dposi,r2posr,r2posi,ten,teste,testeo,x
...
c  complex(knd) scalars and arrays
     ...
     1               doldd,dc01,psum,pdsum,qndsum,qdsum,qnsum,
     2               qsum,r1c,r1dc,r2c,r2dc,spsum,spdsum,wronc,
...
80      nsqnsumr=0
     ...
     1                                        (qndsumpi/aimag(qndsum))))
        ...
        iqndsum=iqm1+idc01-ifajo(l-m+1)-itermpq+iterm
          if(iqndsum-iqdsum.lt.-ndec) then
          qndsum=(0.0e0_knd,0.0e0_knd)
          else
...
c  complex(knd) scalars
     ...
     1               sumi,sumdi,sumr,sumdr,termi,termdi,termr,termdr
c  complex(knd) arays
...
c               ndec: precision for real(kind)
c
...
c
     ...
     1                     ndec,ndec1,maxd,ioprad,minacc,eigest,eigmat,
     2                     lipl,lips,liplp,match,eigp,eign,kindd,kindq,
...
c                       (for knd1 arithmetic)
c               ndec  : number of decimal digits for real(knd)
c               ndec1 : number of decimal digits for real(knd1)
...
c                         desired; set equal to 1 if only radial
c                         functions of the first kind are desired;
c                         set equal to 2 if their first derrivatives
c                         are also desired
c               iopang  : set equal to zero if no angular functions
c                         are desired; set equal to 1 if only angular
...
c                        arithmetic
c               nex    : maximum exponent for real(knd) arithmetic
...
c  real(knd) scalars and arrays
     ...
     4            teste,testeo,test1,twom,twomi,x,x2,xsp1,zi,zl,zu
            ...
            p(2)=alpha(1)*etai*p(1)
              ...
              p(il+2)=alpha(il+1)*etai*p(il+1)+beta(il+1)*p(il)
                ...
                pint4(il+2)=pint4(il+2)+coef4*p(il+2)  
                  if(abs(real(pint1(il+1))).gt.test1.or.
     1               abs(real(pint2(il+1))).gt.test1.or.
     2               abs(real(pint4(il+2))).gt.test1) then
                  limi=il
                  go to 160
                  end if
                  if(abs(aimag(pint1(il+1))).gt.test1.or.
     1               abs(aimag(pint2(il+1))).gt.test1.or.
     2               abs(aimag(pint4(il+2))).gt.test1) then
                  limi=il
                  go to 160
                  end if
                end if  
...
190	continue
     ...
     1                /alpha(il+1)
            if(real(pint3(il+1)).gt.test1.or.aimag(pint3(il+1)).gt.
     1        test1) then
            limi=il
            go to 210
            end if     
...
c                        calculated
c               nex    : largest exponent for real(knd) arithmetic
c
