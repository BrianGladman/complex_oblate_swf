...
if (warn) then
              ...
              naccetamax=0
              if(ioprad.eq.2.and.li.le.lipl.and.match(2).ne.0.0e0_knd.and.ix.eq.1) nar(li-1)=naccrp
              if(ioprad.eq.2.and.li.le.lipl.and.match(2).ne.0.0e0_knd.and.ix.eq.1) nar(li)=naccr
              if(ioprad.eq.2.and.(li.gt.lipl.or.match(2).eq.0)) nar(li)=naccr
              if(ioprad.eq.1) nar(li)=naccr1             
              naccrp=naccr
...
if (debug) then
                if(iopang.eq.1) write(50,1425) barg(jarg),naccs(jarg)
...
if (output) then
                ...
                if(iopang.eq.2) write(30,1470) barg(jarg),s1c(jarg),is1e(jarg),s1dc(jarg),is1de(jarg),naccs(jarg),naccds(jarg)
