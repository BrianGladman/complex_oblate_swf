...
program coblfcn
!      version 1.06 April 2021
!
    use param
!
!  Originally developed over 5 years ago by arnie lee van buren and updated and
!  improved several times since then. For more information see the github repository:
!  GitHub.com/MathieuandSpheroidalWaveFunctions/complex_oblate_swf. Especially see
...
!                   should be an even integer.If lnum is chosen to
...
!
!       line 3:
!          c      : complex value of the size parameter (= kd/2,
!                   where k = the complex wavenumber and d =
...
!
        call main (mmin,minc,mnum,lnum,c,ioprad,iopang,iopnorm,minacc, &
                   x,ngau,step1,nstep1,step2,nstep2,step3,nstep3, &
                   ioparg,arg1,darg,narg,maxd,maxdr,maxe,maxe2,maxint, &
...
!  real(knd) and complex(knd) scalars
        real(knd) aj1,aj2,apcoef,apcoefn,api,arg1,c,coefn,coefme, &
                  coefmo,darg,dec,em,etaval,factor,factor1,pcoefn,pi, &
                  qdm0,qdm1,qm0,qm1,rm,rm2,step1,step2,step3,ten,termpq, &
                  teste,testeo,t1,t2,t3,t4,t5,t6,t7,wm,x,xb,xbninp,xl, &
                  xhigh,xlow,xneu
...
!  integer arrays with dimension lnum
        integer   ir1e(lnum),ir1de(lnum),ir2e(lnum),ir2de(lnum), &
                  match(lnum),iqdl(lnum),iql(lnum),ieigt(lnum)
!
              ...
              p(il+2)=alpha(il+1)*etai*p(il+1)+beta(il+1)*p(il)
                ...
                pint4(il+2)=pint4(il+2)+coef4*p(il+2)
                  if(abs(real(pint1(il+1))).gt.test1.or. &
                     abs(real(pint2(il+1))).gt.test1.or. &
                     abs(real(pint4(il+2))).gt.test1) then
                  limi=il
                  go to 160
                  end if
                  if(abs(aimag(pint1(il+1))).gt.test1.or. &
                     abs(aimag(pint2(il+1))).gt.test1.or. &
                     abs(aimag(pint4(il+2))).gt.test1) then
                  limi=il
                  go to 160
                  end if
                end if
